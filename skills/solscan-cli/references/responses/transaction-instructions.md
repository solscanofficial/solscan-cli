...
| `outer_program_id` | string \| null | For an inner instruction, the `program_id` of the top-level instruction that triggered it via CPI. `null` for top-level instructions. |
| `outer_ins_index` | number | For an inner instruction, the `ins_index` of the top-level instruction that triggered it. `-1` for top-level instructions. |
...
- `outer_program_id`/`outer_ins_index` point back at the parent instruction instead of being `null`/`-1`.
...
| `outer_ins_index` | number | Same semantics as the parent instruction's `outer_ins_index`. |
| `outer_program_id` | string \| null | Same semantics as the parent instruction's `outer_program_id`. |
...
| `outer_program_id` | string \| null | Program that triggered this transfer via CPI, if any. |
| `ins_index` | number | Index of the instruction that produced this transfer. |
| `outer_ins_index` | number | Index of the outer instruction, if this transfer came from a CPI. |
